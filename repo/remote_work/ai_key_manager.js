...
function validateKey(provider, keyValue) {
  const key = String(keyValue || '').trim();
  if (!key) throw new Error('API key tidak boleh kosong.');
  if (provider === 'gemini' && !/^AQ\.[A-Za-z0-9_-]+$/.test(key)) {
    throw new Error('Format key Gemini tidak valid. Gunakan key baru yang diawali AQ.');
  }
  return key;
}
...
function addKey(provider, keyValue) {
  ...
  keyValue = validateKey(provider, keyValue);
  const data = loadKeys();
...
function removeKey(provider, selector) {
  ...
  const i = /^\d+$/.test(normalized)
    ? Number(normalized) - 1
    : keys.findIndex(k => k.key === normalized);
...
function getEnvKey(provider) {
  if (provider === 'gemini') {
    ...
    return /^AQ\.[A-Za-z0-9_-]+$/.test(key) ? key : null;
  }
