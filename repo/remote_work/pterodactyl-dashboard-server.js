...
function readQuotes() {
    try { return JSON.parse(fs.readFileSync(QUOTES_FILE, "utf8")); } catch { return []; }
    }
    ...
    if (GK) {
      try {
        ...
        if (r.ok){const d=await r.json();const t=stripMd(d?.candidates?.[0]?.content?.parts?.[0]?.text);if(t&&t.startsWith("{")){return JSON.parse(t);}}
